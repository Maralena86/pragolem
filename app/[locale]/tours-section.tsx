 ...
 */
export function ToursSection({ t, tours, languageMap, guideNameBySlug, locale }: ToursSectionProps) {
  return (
    <section id="tours" className="space-y-5">
      <div className="space-y-2">
        <h2 className="text-2xl font-semibold tracking-tight">{t("sections.tours.title")}</h2>
          ...
          <Card key={tour.id}>
            ...
            <CardContent>
              <ul className="space-y-1 text-muted-foreground">
                <li>
                  {t("tourCard.durationLabel")}: {tour.attributes.durationDisplay}
                </li>
                <li>
                  {t("tourCard.languagesLabel")}:{" "}
                  {tour.attributes.languages.map((code) => languageMap[code] ?? code).join(", ")}
                </li>
                <li>
                ...
                <li>
                  ...
                  {(tour.relationships.guide.data.slug && guideNameBySlug.get(tour.relationships.guide.data.slug)) ?? t("tourCard.fallbackGuide")}
                </li>
              </ul>
            </CardContent>
            <CardFooter>
              <Button variant="outline" asChild>
                <Link href={{ pathname: "/tours/[slug]", params: { slug: tour.slug[locale] } }}>
                  {t("tourCard.cta")}
                </Link>
              </Button>
            </CardFooter>
