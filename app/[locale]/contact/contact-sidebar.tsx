import { RiArrowRightUpLine, RiMailLine, RiMapPinLine, RiPhoneLine, RiWhatsappLine } from "@remixicon/react";
        ...
        <CardContent className="space-y-2">
          <Button variant="outline" className="w-full justify-start" asChild>
            <a href={phoneHref}>
              ...
              {t("quickActions.call")} · {phoneDisplay}
            </a>
          </Button>
          <Button variant="outline" className="w-full justify-start" asChild>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {socialLinks.map((socialLink) => (
            <Button key={socialLink.url} variant="outline" className="w-full justify-between" asChild>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="rounded-none border border-dashed p-3 text-sm text-muted-foreground">
            <p className="font-medium text-foreground">{mapPoint?.name ?? t("map.fallbackName")}</p>
            <p>{mapPoint?.address ?? t("map.fallbackAddress")}</p>
            <p>{t("map.placeholderNote")}</p>
          </div>
          <Button variant="outline" className="w-full justify-start" asChild>
            <a href={mapPoint?.googleMapsUrl ?? "https://maps.google.com"} target="_blank" rel="noreferrer">
