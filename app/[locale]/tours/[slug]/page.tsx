 ...
 */
function getTourAggregateRating(reviews: Review[]): { ratingValue: number; reviewCount: number } {
  if (reviews.length === 0) {
    return { ratingValue: 0, reviewCount: 0 };
  }

  const total = reviews.reduce((sum, review) => sum + review.attributes.rating, 0);
  return {
...
export default async function TourDetailPage({
  ...
  const [guide, allTours, reviews, faqItems] = await Promise.all([
    getGuideBySlug(tour.relationships.guide.data.slug!, localeCode),
    getTours(localeCode),
    getReviews(localeCode, tour.slug[localeCode]),
    getFAQItems(localeCode, getFaqCategoryForTour(tour)),
  ]);

  const relatedToursBySlug = new Map(allTours.map((candidate) => [candidate.slug[localeCode], candidate]));
  ...
  const featuredReviews = reviews
    .toSorted((a, b) => Number(b.attributes.isRealReview) - Number(a.attributes.isRealReview))
    .slice(0, 4);
  const itineraryStops = tour.attributes.itinerary.toSorted((a, b) => a.order - b.order);
  const longDescriptionParagraphs = splitParagraphs(tour.attributes.longDescription);
  ...
  const tourAbsoluteUrl = buildTourUrl(getSiteUrl(), localeCode, tour.slug[localeCode]);
  ...
  return (
    <>
      ...
      <main id="main-content" className="mx-auto flex w-full max-w-6xl flex-col gap-12 px-4 py-10 sm:px-6 sm:py-12">
        ...
        <TourGuideSection t={t} guide={guide} />
        <TourReviewsSection t={t} featuredReviews={featuredReviews} aggregate={aggregate} />
        <TourRelatedSection t={t} relatedTours={relatedTours} localeCode={localeCode} />
